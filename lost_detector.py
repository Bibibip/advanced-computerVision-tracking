...
def check_lost_status(state, cx, cy, current_sec_exact):
    ...
    if state['owner_id'] is not None:
        if state['status'] == 'held':
            ...
            if 'separate_start' not in state:
                state['separate_start'] = current_sec_exact

            elif current_sec_exact - state['separate_start'] > 1.0:
